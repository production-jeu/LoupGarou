...
namespace Wolf
{
    ...
    public class TimeManager : MonoBehaviour
    {
        ...
        Vector3 rotSoleilDebutJour = new Vector3(-25, 0, 0);
        Color32 ambiantLightJour = new Color32(15, 15, 15, 255);
        Color32 ambiantLightNuit = new Color32(1, 1, 1, 255);

        private IntEvent OnChangementHeure;

        private void FixedUpdate()
        {
            soleil.transform.Rotate(new Vector3(0.01f, 0, 0));
        }

        IEnumerator FadeAmbiantToDay(Color32 colInitiale, Color32 colFinale)
        {
            for (int x = 0; x < 400; x++)
            {
                RenderSettings.ambientSkyColor = Color32.Lerp(colInitiale, colFinale, (float)x/400);
                RenderSettings.ambientEquatorColor = Color32.Lerp(colInitiale, colFinale, (float)x / 400);
                soleil.shadowStrength = Mathf.Lerp(0.95f, 0.3f, (float)x/400);
                yield return null;
            }
            RenderSettings.ambientSkyColor = colFinale;
            RenderSettings.ambientEquatorColor = colFinale;
        }
        ...
        {
            soleil.transform.rotation = Quaternion.Euler(rotSoleilDebutJour);
            StartCoroutine(FadeAmbiantToDay(ambiantLightNuit, ambiantLightJour));
            heure = 7;
